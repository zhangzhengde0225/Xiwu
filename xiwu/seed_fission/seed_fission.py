...
class SeedFission(BaseQADatasetSaver):
    ...
    def __call__(self, seed, max_entities=2000, **kwargs):
...
@dataclass
class Args:
    domain: str = field(default='High Energy Physics', metadata={'help': 'domain to generate QA dataset'})
    ...
    max_entities: int = field(default=1000, metadata={'help': 'max number of entities to generate for all runs'})
    output_dir: str = field(default=f'{here}/post_fission_data', metadata={'help': 'output file'})
