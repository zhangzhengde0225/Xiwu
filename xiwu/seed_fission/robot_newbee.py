...
class NewbeeBot(HepAILLM):
    ...
    def newbee_prompt_engineering(self, input):
        ...
        num = np.random.normal(loc=7, scale=5, size=None)
...
Privoide them in JSON format, for example: {example}, only output one JSON object.
{self.prompt_lang}
...
"""
    ...
    def input2questions(self, input, **kwargs):
        ...
        questions = self.__call__(prompt, sys_prompt=None, temperature=0.5, **kwargs)
        return questions
