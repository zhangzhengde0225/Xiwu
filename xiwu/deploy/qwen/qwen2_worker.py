...
from xiwu.modules.models.qwen2 import Qwen2
...
class WorkerModel(BaseWorkerModel):
    def __init__(self, args, **kwargs):
        ...
        self.xmodel: XBaseModel = Qwen2(args=args, **kwargs)
            ...
            if '\n' in json_buffer:
                ...
                for line in lines[:-1]:
                    ...
                    if line.startswith('data: '):
                        json_str = line[len('data: '):]  # 去掉 'data: ' 前缀
    ...
    def inference_ori(self, **kwargs):
        ...
        prompt = self.xmodel.oai_messages2prompt(full_messages)
...
class ModelArgs(BaseArgs):
    name: str = "Qwen2-72B-Instruct"  # worker的名称，用于注册到控制器
    model_path: str = '/data1/sqr/Models/Qwen2-72B-Instruct'
    # 其他参数
...
class WorkerArgs:
    ...
    description: str = 'Qwen2 from Qwen'  # 模型的描述
    author: str = 'Qwen'  # 模型的作者
...
def run_worker(**kwargs):
    ...
    if worker_args.test:
        ...
        if not stream:
            ...
            # 删除前两个单词及其后的换行符
            response = ' '.join(response.split()[2:]).replace('\n','')
            print(response)
        else:
            for i in chatcmpl:
                sys.stdout.write(i)
